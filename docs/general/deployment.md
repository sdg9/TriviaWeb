...
*Step 1:* Create a _Procfile_ with the following line: `web: yarn start:prod`. We do this because Heroku runs `yarn start` by default, so we need this setting to override the default run command.
...
*Step 1:* Run `yarn` to install dependencies, then `yarn build` to create the `./build` folder.
